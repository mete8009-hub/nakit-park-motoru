...
def build_quote_monitor(quotes: pd.DataFrame, instruments: pd.DataFrame) -> pd.DataFrame:
    ...
    )[["instrument_id", "instrument_name", "instrument_type", "provider_name", "tenor_days", "instrument_family"]].copy()
    ...
    for col in ["instrument_name", "instrument_type", "provider_name", "tenor_days", "instrument_family"]:
        q_col = f"{col}_q"
        if col not in latest_q.columns and q_col in latest_q.columns:
            latest_q[col] = latest_q[q_col]
        elif col in latest_q.columns and q_col in latest_q.columns:
            latest_q[col] = latest_q[col].where(latest_q[col].notna(), latest_q[q_col])

    latest_q["instrument_name"] = latest_q.get("instrument_name", pd.Series(index=latest_q.index, dtype="object")).fillna("")
    latest_q["instrument_type"] = latest_q.get("instrument_type", pd.Series(index=latest_q.index, dtype="object")).fillna("")
    latest_q["provider_name"] = latest_q.get("provider_name", pd.Series(index=latest_q.index, dtype="object")).fillna("")
    latest_q["tenor_days"] = pd.to_numeric(latest_q.get("tenor_days", np.nan), errors="coerce")
    latest_q["instrument_family"] = latest_q.get("instrument_family", pd.Series(index=latest_q.index, dtype="object")).fillna("")
    latest_q["instrument_family"] = latest_q["instrument_family"].replace("", np.nan).fillna(latest_q["instrument_type"]).fillna("Diğer")
    ...
    latest_q["allowed_age_min"] = latest_q["instrument_type"].map(quote_stale_minutes).fillna(6 * 60)
    ...
    sort_cols = [c for c in ["instrument_family", "tenor_days", "instrument_name"] if c in latest_q.columns]
    if sort_cols:
        latest_q = latest_q.sort_values(sort_cols, na_position="last")
    return latest_q.reset_index(drop=True)
